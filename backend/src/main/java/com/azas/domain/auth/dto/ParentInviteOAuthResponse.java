...
@ApiModel(description = "부모 초대코드 기반 소셜 로그인 응답")
@Getter
@RequiredArgsConstructor(access = AccessLevel.PRIVATE)
public class ParentInviteOAuthResponse {
    ...
    @ApiModelProperty(
            value = "부모 관계가 등록된 자녀 정보",
            required = true
    )
    private final ParentInviteChildResponse child;

    @ApiModelProperty(
            value = "수락 완료된 가족 초대 정보",
            required = true
    )
    private final ParentInviteInvitationResponse invitation;

    public static ParentInviteOAuthResponse from(
            ParentInviteOAuthResult result
    ) {
        AuthTokenPair tokenPair =
                result.getTokenPair();

        return new ParentInviteOAuthResponse(
                ...
                ParentInviteChildResponse.from(result),
                ParentInviteInvitationResponse.from(result)
        );
    }
}