...
class AccountOpenControllerTest {
    @Mock AccountOpenService service;
    ...
    @Test void opensMockAccount() throws Exception {
        when(resolver.resolveMemberId("Bearer token")).thenReturn(1L);
        when(service.open(eq(1L), any())).thenReturn(new AccountOpenResult(
                10L, "PARENT", null, 2L, "KB국민은행", "KB Mock 통장",
                "123-456-789012", "DEMAND_DEPOSIT", BigDecimal.ZERO,
                true, null, LocalDateTime.of(2026, 8, 13, 5, 0)));
        mockMvc.perform(post("/api/v1/accounts/open")
                        .header("Authorization", "Bearer token")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
                          {"owner_type":"PARENT","child_id":null,"financial_product_id":2,"initial_deposit_amount":0,"goal":null}
                          """))
                .andExpect(status().isCreated())
                .andExpect(jsonPath("$.account_id").value(10L))
                .andExpect(jsonPath("$.account_number").value("123-456-789012"))
